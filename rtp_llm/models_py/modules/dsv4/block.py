...
class Block(nn.Module):
    ...
    def forward_decode(
        ...
        # DSpARK supplies a non-causal attention callback and intentionally
        # bypasses the regular CSA decode adapter for its draft block.
        if (
            attn_fn is None
            and mega_csa is not None
            and mega_csa.supports_decode_shape(x, attn_metadata)
        ):
