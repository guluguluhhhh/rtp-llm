...
class MegaMoeFrontAdapterTest(unittest.TestCase):
    def test_front_is_automatically_attached_to_mega_se(self) -> None:
        moe = SimpleNamespace(
        ...
        adapter = object()
        with mock.patch(
            "rtp_llm.models_py.modules.dsv4.moe.moe_layer.MegaMoeFrontAdapter",
            return_value=adapter,
        ) as adapter_cls:
            MoE.enable_mega_front(moe, "hc", "norm")

        self.assertIs(moe._mega_front, adapter)
        adapter_cls.assert_called_once_with(moe, "hc", "norm")

    def test_front_is_not_attached_to_non_mega_strategy(self) -> None:
        moe = SimpleNamespace(
            _strategy=SimpleNamespace(name="local_loop"),
            _mega_front=None,
        )
        with mock.patch(
            "rtp_llm.models_py.modules.dsv4.moe.moe_layer.MegaMoeFrontAdapter"
        ...
        self.assertIsNone(moe._mega_front)
